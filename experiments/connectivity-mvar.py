...
images = []
for j, (delta_plot, theta_plot, alpha_plot, beta_plot) in enumerate(zip(delta_connectivity, theta_connectivity, alpha_connectivity, beta_connectivity)):
    for i, (freq_label, plot) in enumerate(zip(['Delta (1-4 Hz)', 'Theta (4-8 Hz)', 'Alpha (8-13 Hz)', 'Beta (13-30 Hz)'], [delta_plot, theta_plot, alpha_plot, beta_plot])):
        if j == 0:
...
fig.colorbar(images[0], ax=axs, fraction=0.036, pad=0.012)